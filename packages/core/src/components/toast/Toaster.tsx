import { removeToast, toasts } from "./toast-store";
...
export const Toaster = ({ position = "bottom-right" }: ToasterProps) => {
  return (
    <div
      ...
      class={[
        "pointer-events-none fixed z-[120] grid w-[calc(100%-2rem)] max-w-sm gap-2",
        POSITION_CLASS[position],
      ].join(" ")}
    >
      {toasts.value.map((toast) => {
        const isAssertive = toast.variant === "destructive" || toast.variant === "error";
        return (
          <div
            key={toast.id}
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="grid min-w-0 flex-1 gap-1">
                ...
                <div class="text-[10px] uppercase tracking-wide text-muted-foreground/90">
                  {LABEL[toast.variant ?? "default"] ?? LABEL.default}
                </div>
              </div>
            </div>
          </div>
