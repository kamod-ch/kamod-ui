...
const AiPromptSourcesMenu = () => {
  ...
  const sourceSwitchWrapClass =
    "landing-ai-sources-switch-wrap origin-right scale-90 shrink-0 cursor-pointer";
  const sourceSwitchClass =
    "landing-ai-sources-switch pointer-events-none opacity-100 transition-colors data-[state=checked]:bg-foreground data-[state=checked]:saturate-100 data-[state=unchecked]:bg-muted-foreground/35";
      ...
      >
        <div class="flex flex-col gap-px px-0.5">
          ...
          >
            ...
            <span aria-hidden="true" class={sourceSwitchWrapClass}>
              <Switch
                checked={webSearch}
                size="sm"
                tabIndex={-1}
...
type AddContextPageId = (typeof ADD_CONTEXT_PAGES)[number]["id"];
...
const AiPromptAddContextMenu = ({
  ...
  selectedIds: ReadonlySet<AddContextPageId>;
  onSelectPage: (id: AddContextPageId) => void;
}) => {
          ...
          {filtered.length === 0 ? (
            ...
            <div class="flex flex-col gap-px">
              {filtered.map((page) => {
                ...
                const isActive = selectedIds.has(page.id);
                return (
                  <DropdownItem
                    key={page.id}
...
const KitchenSinkAiPromptComposer = () => {
  const [contextIds, setContextIds] = useState<AddContextPageId[]>([]);

  const addContextPage = (id: AddContextPageId) => {
    setContextIds((prev) => (prev.includes(id) ? prev : [...prev, id]));
  };

  const removeContextPage = (id: AddContextPageId) => {
    setContextIds((prev) => prev.filter((pageId) => pageId !== id));
  };

  const selectedContextPages = contextIds
    .map((id) => ADD_CONTEXT_PAGES.find((page) => page.id === id))
    .filter((page): page is (typeof ADD_CONTEXT_PAGES)[number] => page != null);

  return (
    <div class="landing-ai-prompt" data-testid="kitchen-sink-ai-prompt">
      <AiPromptAddContextMenu selectedIds={new Set(contextIds)} onSelectPage={addContextPage} />
      <div class="landing-ai-prompt-field">
        {selectedContextPages.length > 0 ? (
          <div
            class="landing-ai-prompt-context-badges flex flex-wrap gap-1"
            aria-label="Added context"
          >
            {selectedContextPages.map((page) => {
              const Icon = page.Icon;
              return (
                <Badge
                  key={page.id}
                  variant="outline"
                  size="xs"
                  class="gap-1 pe-0.5 font-normal leading-tight"
                >
                  <Icon class="size-3 shrink-0 text-muted-foreground" strokeWidth={1.75} />
                  <span class="min-w-0">{page.label}</span>
                  <button
                    type="button"
                    class="text-muted-foreground hover:text-foreground shrink-0 rounded-sm p-0.5 outline-none focus-visible:ring-2 focus-visible:ring-ring/50"
                    aria-label={`Remove ${page.label}`}
                    onClick={() => removeContextPage(page.id)}
                  >
