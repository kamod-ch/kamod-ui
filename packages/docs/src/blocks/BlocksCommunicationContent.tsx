...
type CommunicationBlock = (typeof communicationBlocks)[number];

export const BlocksCommunicationContent = () => (
  <DocsShell
    ...
    mainContent={
      <section class="docs-components-overview blocks-sidebar-page">
        <header class="blocks-hero">
          ...
          <FeaturedBlockChips active="communication" />
        </header>
...
const buildFileTree = (files: CommunicationBlock["files"]) => {
...
export const CommunicationBlocksPreviewContent = ({
  id,
  mode = "desktop",
}: {
  id?: string;
  mode?: "desktop" | "mobile" | "collapsed";
}) => {
  const block = communicationBlocks.find((item) => item.id === id) ?? communicationBlocks[0];
