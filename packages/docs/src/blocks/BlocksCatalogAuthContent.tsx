...
const buildFileTree = (files: CatalogAuthBlock["files"]) => {
...
const BlockCard = ({ block }: { block: CatalogAuthBlock }) => {
  ...
  const previewProps = catalogAuthPreviewProps[block.id] ?? {};
  const Preview = () => {
...
export const CatalogAuthBlocksPreviewContent = ({ id }: { id?: string }) => {
  const block = catalogAuthBlocks.find((item) => item.id === id) ?? catalogAuthBlocks[0];
