    ...
    <form ref={formRef} onSubmit={submit} class="grid gap-6" noValidate>
      ...
      <div class="grid gap-4">
        <div class="grid gap-2">
          <Label for="login-04-email">Email</Label>
          <Input
            id="login-04-email"
            name="email"
            type="email"
            ...
            aria-describedby={errors.email ? "login-04-email-error" : undefined}
            onInput={(event) =>
              setValues((current) => ({ ...current, email: event.currentTarget.value }))
            }
          />
          {errors.email ? (
            <p id="login-04-email-error" class="text-sm text-destructive">
              {errors.email}
            </p>
          ) : null}
        </div>
        <div class="grid gap-2">
          <div class="flex items-center justify-between gap-3">
            <Label for="login-04-password">Password</Label>
            <a
          ...
          <Input
            id="login-04-password"
            name="password"
            type="password"
            autocomplete="current-password"
            value={values.password}
            aria-invalid={Boolean(errors.password)}
            aria-describedby={errors.password ? "login-04-password-error" : undefined}
            onInput={(event) =>
              setValues((current) => ({ ...current, password: event.currentTarget.value }))
            }
          />
          {errors.password ? (
            <p id="login-04-password-error" class="text-sm text-destructive">
              {errors.password}
            </p>
          ) : null}
        </div>
