    ...
    <form ref={formRef} onSubmit={submit} class="grid gap-6" noValidate>
      ...
      <div class="grid gap-4">
        <div class="grid gap-2">
          <Label for="signup-01-name">Name</Label>
          <Input
            id="signup-01-name"
            name="name"
            autocomplete="name"
            ...
            aria-describedby={errors.name ? "signup-01-name-error" : undefined}
            onInput={(event) =>
              setValues((current) => ({ ...current, name: event.currentTarget.value }))
            }
          />
          {errors.name ? (
            <p id="signup-01-name-error" class="text-sm text-destructive">
              {errors.name}
            </p>
          ) : null}
        </div>
        <div class="grid gap-2">
          <Label for="signup-01-email">Email</Label>
          <Input
            id="signup-01-email"
            name="email"
            type="email"
            ...
            aria-describedby={errors.email ? "signup-01-email-error" : undefined}
            onInput={(event) =>
              setValues((current) => ({ ...current, email: event.currentTarget.value }))
            }
          />
          {errors.email ? (
            <p id="signup-01-email-error" class="text-sm text-destructive">
              {errors.email}
            </p>
          ) : null}
        </div>
        <div class="grid gap-2">
          <Label for="signup-01-password">Password</Label>
          <Input
            ...
            aria-describedby={errors.password ? "signup-01-password-error" : undefined}
            onInput={(event) =>
              setValues((current) => ({ ...current, password: event.currentTarget.value }))
            }
          />
          {errors.password ? (
            <p id="signup-01-password-error" class="text-sm text-destructive">
              {errors.password}
            </p>
          ) : null}
        </div>
        <div class="flex items-start gap-3">
          <Checkbox
            ...
            aria-describedby={
              errors.terms ? "signup-01-terms-error signup-01-terms-help" : "signup-01-terms-help"
            }
            onCheckedChange={(value) => setAccepted(value === true)}
          />
          <div class="grid gap-1.5 leading-none">
            <Label for="signup-01-terms">Accept terms</Label>
            <p id="signup-01-terms-help" class="text-sm text-muted-foreground">
            ...
            {errors.terms ? (
              <p id="signup-01-terms-error" class="text-sm text-destructive">
                {errors.terms}
              </p>
            ) : null}
          </div>
