    ...
    <form ref={formRef} onSubmit={submit} class="grid gap-6" noValidate>
      ...
      <div class="grid gap-4">
        <div class="grid gap-2">
          <Label for="signup-04-name">Name</Label>
          <Input
            id="signup-04-name"
            name="name"
            autocomplete="name"
            ...
            aria-describedby={errors.name ? "signup-04-name-error" : undefined}
            onInput={(event) =>
              setValues((current) => ({ ...current, name: event.currentTarget.value }))
            }
          />
          {errors.name ? (
            <p id="signup-04-name-error" class="text-sm text-destructive">
              {errors.name}
            </p>
          ) : null}
        </div>
        <div class="grid gap-2">
          <Label for="signup-04-email">Email</Label>
          <Input
            id="signup-04-email"
            name="email"
            type="email"
            ...
            aria-describedby={errors.email ? "signup-04-email-error" : undefined}
            onInput={(event) =>
              setValues((current) => ({ ...current, email: event.currentTarget.value }))
            }
          />
          {errors.email ? (
            <p id="signup-04-email-error" class="text-sm text-destructive">
              {errors.email}
            </p>
          ) : null}
        </div>
        <div class="grid gap-2">
          <Label for="signup-04-password">Password</Label>
          <Input
            id="signup-04-password"
            name="password"
            type="password"
            autocomplete="new-password"
            value={values.password}
            aria-invalid={Boolean(errors.password)}
            aria-describedby={errors.password ? "signup-04-password-error" : undefined}
            onInput={(event) =>
              setValues((current) => ({ ...current, password: event.currentTarget.value }))
            }
          />
          {errors.password ? (
            <p id="signup-04-password-error" class="text-sm text-destructive">
              {errors.password}
            </p>
          ) : null}
        </div>
        <div class="flex items-start gap-3">
          <Checkbox
            ...
            aria-describedby={
              errors.terms ? "signup-04-terms-error signup-04-terms-help" : "signup-04-terms-help"
            }
            onCheckedChange={(value) => setAccepted(value === true)}
          />
          <div class="grid gap-1.5 leading-none">
            <Label for="signup-04-terms">Accept terms</Label>
            <p id="signup-04-terms-help" class="text-sm text-muted-foreground">
            ...
            {errors.terms ? (
              <p id="signup-04-terms-error" class="text-sm text-destructive">
                {errors.terms}
              </p>
            ) : null}
          </div>
