...
async function collectSectionIds(page: Page): Promise<string[]> {
  return page
    .locator("main section.docs-section[id]")
    .evaluateAll((els) => els.map((el) => el.getAttribute("id") ?? "").filter(Boolean));
}
...
test("docs /docs/components full link audit (writes tmp report)", async ({ page, baseURL }) => {
  ...
  try {
    ...
    const gridHrefs = await page
      .locator("a.docs-component-item[href]")
  ...
  expect(auditHasBlockers(), `Docs audit reported blockers — see ${REPORT_PATH}`).toBe(false);
});
